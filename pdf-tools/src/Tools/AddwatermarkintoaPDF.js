...
function AddwatermarkintoaPDF() {
  ...
  const applyWatermark = async () => {
    if (pdfData && watermarkText) {
      const existingPdfBytes = pdfData;
      const pdfDoc = await PDFDocument.load(existingPdfBytes);
      const pages = pdfDoc.getPages();
      const fontSize = 100; // Increase font size for larger text
  
      pages.forEach((page) => {
        const { width, height } = page.getSize();
  
        page.drawText(watermarkText, {
          x: width / 2 - (watermarkText.length * fontSize) / 8, // Center horizontally
          y: height / 2 - fontSize / 0.4, // Center vertically
          size: fontSize,
  ...
  return (
    <div className="pdf-container">
      ...
      <div className="container">
        <h1>Add watermark into a PDF</h1>
        <p>
...
<br/>
            <button className="btn-77" onClick={applyWatermark} disabled={!previewUrl || !watermarkText}>
        ...
        <br/>
        <button className="btn-77" onClick={handleDownload}>Download Watermarked PDF</button>
      </div>
    </div>
  );
}

export default AddwatermarkintoaPDF;
